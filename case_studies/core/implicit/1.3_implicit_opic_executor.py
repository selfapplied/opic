...
class OpicExecutor:
        ...
        try:
            ...
            try:
                ...
                if not parser_path.exists():
                    parser_path = self.project_root / "generate.py"
                if parser_path.exists():
                    ...
                    raise ImportError(f"Could not find parser.py at {parser_path}")
    ...
    def _load_opic_systems(self):
        ...
        if core_dir.exists():
            for core_file in core_dir.glob("*.ops"):
    ...
    def _load_ops_file(self, file_path: Path):
        ...
        # parse_ops now returns 5 values: defs, voices, includes, boundary_matrix, symbol_list
        result = parse_ops(content, file_path)
        if len(result) == 5:
            defs, voices, includes, boundary_matrix, symbol_list = result
        else:
            # Fallback for older parse_ops signature
            defs, voices, includes = result[:3]
    ...
    def _discover_relevant_voices(self, context: str, current_step: str = None, visited: set = None) -> List[str]:
        """
        Discover relevant voices using field operations - semantic matching via field coherence
        """
        if visited is None:
            visited = set()
        
        # Use field operations to find semantically similar voices
        context_text = current_step if current_step else context
        if not context_text or context_text in visited:
            return []
        
        visited.add(context_text)
        
        # Compute field for context using aperture chain
        try:
            context_field = self._call_primitive("aperture.chain", {"text": context_text})
            context_phi_k = context_field.get("aperture", {}).get("discourse", {}).get("phi_k", 0.0)
        except:
            context_phi_k = 0.0
        
        # Find voices with similar field properties
        discovered = []
        for voice_name, voice_body in self.voices.items():
            # Skip if already visited
            if voice_name in visited:
                continue
            
            # Extract voice name tokens for semantic matching
            voice_tokens = self._string_tokens(voice_name)
            context_tokens = self._string_tokens(context_text)
            
            # Simple semantic similarity via token overlap
            if voice_tokens and context_tokens:
                overlap = len(set(voice_tokens) & set(context_tokens))
                if overlap > 0:
                    discovered.append(voice_name)
        
        return discovered[:5]  # Return top 5 matches
    
    def _enhance_with_discovered(self, result: Any, discovered_voices: List[str], context: str) -> Any:
        """
        Enhance result using discovered voices via field operations
        Uses field.coherence to compose discovered voices with result
        """
        if not discovered_voices or not result:
            return result
        
        # Use field coherence to enhance result with discovered voices
        try:
            # Compute coherence between result and discovered voices
            result_text = str(result) if not isinstance(result, dict) else str(result.get("text", result))
            
            # Build enhancement chain from discovered voices
            enhanced = result
            for voice_name in discovered_voices[:3]:  # Use top 3 discovered voices
                if voice_name in self.voices:
                    # Use field.coherence to check if voice enhances result
                    coherence = self._call_primitive("field.coherence", {
                        "text1": result_text,
                        "text2": voice_name
                    })
                    
                    # If coherence is high, compose voice with result
                    if isinstance(coherence, (int, float)) and coherence > 0.5:
                        # Compose: result -> voice -> enhanced
                        try:
                            voice_result = self._execute_opic_chain([voice_name], {"input": result})
                            if voice_result:
                                enhanced = voice_result
                        except:
                            pass  # Skip if composition fails
            
            return enhanced
        except:
            # Fallback: return original result if enhancement fails
            return result
...
'''
        ...
        self.primitives = {
            ...
            "typst.seal_witness": lambda env: typst_cryptographic_seal({**env, "style": "witness"}),
            "typst.seal_signed": lambda env: typst_cryptographic_seal({**env, "style": "signed"}),
    ...
    def _evaluate_step_token(self, token: str, env: Dict[str, Any], last_result: Any) -> Any:
        """
        Evaluate a single step token using circle diffeomorphism model:
        - Smooth flow: try voice resolution → if fails, rotate to literal
        - No quote walls: literalness is a curvature consequence
        - Two attractors: voice (structured) vs literal (data)
        """
        t = token.strip()
        
        # Legacy quote support (for backward compatibility, but deprecated)
        # Quotes are cliffs - circle diffeomorphisms prefer smooth rotation
        if (t.startswith('"') and t.endswith('"')) or (t.startswith("'") and t.endswith("'")):
            return t[1:-1]
        
        # Circle diffeomorphism: smooth flow toward attractor
        # Step 1: Try voice resolution (structured meaning attractor)
        # Step 2: If fails, smoothly rotate to literal (data attractor)
        result = self._smooth_flow_resolve(t, env, last_result)
        if result != t:  # If smooth flow resolved to something other than literal
            return result
        ...
        # Fallback: return token (smooth rotation to literal attractor)
        return t
    
    def _smooth_flow_resolve(self, token: str, env: Dict[str, Any], last_result: Any) -> Any:
        """
        Circle diffeomorphism: smooth flow toward attractor
        Algorithm:
        1. Compute curvature φₖ(token, context)
        2. Check file existence (voice attractor condition)
        3. If file exists and φₖ > threshold → voice resolution
        4. If file doesn't exist or φₖ < threshold → rotate to literal
        5. No quotes needed - smooth rotation handles everything
        """
        # Step 1: Check for operators (structural singularities - force structure)
        if any(op in token for op in ["->", "+", "."]):
            # Operators force high curvature → try voice resolution first
            # This is the "north pole" of the circle
            voice_result = self._try_voice_resolution(token, env, last_result)
            if voice_result is not None:
                return voice_result
        
        # Step 2: Compute curvature (simplified: check file existence + context)
        curvature = self._compute_token_curvature(token, env)
        
        # Step 3: Try voice resolution if high curvature
        if curvature > 0.1:  # Threshold: φₖ > 0.1 → try structure
            voice_result = self._try_voice_resolution(token, env, last_result)
            if voice_result is not None:
                return voice_result
        
        # Step 4: Smooth rotation to literal attractor (low curvature)
        # This is the "south pole" of the circle
        return token  # Literal interpretation
    
    def _try_voice_resolution(self, token: str, env: Dict[str, Any], last_result: Any) -> Any:
        """
        Try voice resolution (structured meaning attractor)
        Checks: primitives, voices, variables, discovered voices
        """
        # Check primitives first
        if token in self.primitives:
            prim_env = env.copy()
            if last_result is not None:
                prim_env["input"] = last_result
                if isinstance(last_result, (str, Path)):
                    prim_env["path"] = last_result
                    prim_env["file"] = last_result
            prim = self._call_primitive(token, prim_env)
            if prim is not None:
                return prim
        
        # Check voices
        if token in self.voices:
            return self.voices[token]
        
        # Check variables
        if token in env:
            return env[token]
        
        # Try discovered voices (semantic matching)
        discovered = self._discover_relevant_voices(token, visited=set())
        if discovered and discovered[0] in self.voices:
            return self.voices[discovered[0]]
        
        # No voice resolution found → will rotate to literal
        return None
    
    def _compute_token_curvature(self, token: str, env: Dict[str, Any]) -> float:
        """
        Compute field curvature φₖ for token
        Simplified implementation: checks file existence and context
        High curvature → structure likely
        Low curvature → literal likely
        """
        # Check if token looks like a file path
        if token.endswith(".ops") or "/" in token or "\\" in token:
            # Check if file exists
            from pathlib import Path
            if Path(token).exists():
                return 0.9  # High curvature: file exists → structure
        
        # Check if token contains namespace separator (.)
        if "." in token:
            return 0.7  # Medium-high curvature: namespace suggests structure
        
        # Check if token is in environment (variable)
        if token in env:
            return 0.6  # Medium curvature: variable suggests structure
        
        # Check if token matches voice/primitive naming pattern
        if token.replace("_", "").replace(".", "").isalnum():
            return 0.3  # Low-medium curvature: might be structure
        
        # Default: low curvature → literal
        return 0.1
    ...
    def _resolve_steps(self, steps: List[str], inputs: Dict[str, Any]) -> List[Any]:
        ...
        resolved = []
        for step in steps:
            # Legacy quote support (deprecated - quotes are cliffs)
            step_stripped = step.strip()
            if (step_stripped.startswith('"') and step_stripped.endswith('"')) or \
               (step_stripped.startswith("'") and step_stripped.endswith("'")):
                # String literal: leave as-is, will be evaluated during execution
        ...
        try:
            ...
            for i, choice in enumerate(choices):
                ...
                if knowledge_results:
                    knowledge_boost = 1.0  # Start at 1.0 so *= 1.2 gives 20% boost
